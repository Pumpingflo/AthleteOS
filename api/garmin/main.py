...
from garmin_engine import get_recent_activities_from_garmin, sync_workouts_to_garmin
...
class GarminLatestActivityBody(BaseModel):
    garmin_email: str = Field(..., min_length=3)
    garmin_password: str = Field(default="", max_length=500)
    garmin_tokens: str | None = Field(default=None, max_length=600_000)
    ...
    @model_validator(mode="after")
    def _need_password_or_tokens(self) -> "GarminLatestActivityBody":
        pw = (self.garmin_password or "").strip()
        tok = (self.garmin_tokens or "").strip()
        if len(tok) < 200 and not pw:
            raise ValueError(
...
@app.post("/latest_activity")
def latest_activity(
    body: GarminLatestActivityBody,
    authorization: str | None = Header(None),
):
    if not authorization or not authorization.lower().startswith("bearer "):
        raise HTTPException(401, "Authorization: Bearer <Supabase access_token> fehlt")
    ...
    claims = verify_supabase_jwt(token)
    user_id = claims.get("sub")
    log.info("Latest activity request for user %s", user_id)
    try:
        result = get_recent_activities_from_garmin(
            body.garmin_email,
            body.garmin_password,
            garmin_tokens=body.garmin_tokens,
            limit=body.limit,
        ...
        log.exception("Latest activity fetch failed")
        raise HTTPException(500, f"Garmin-Aktivitäten konnten nicht geladen werden: {exc}") from exc
