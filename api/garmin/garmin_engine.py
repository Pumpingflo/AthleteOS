...
def sync_workouts_to_garmin(
    ...
    api = _garmin_login(email, password, garmin_tokens)
...
def _garmin_login(email: str, password: str, garmin_tokens: str | None = None):
    from garminconnect import (
      Garmin,
      GarminConnectAuthenticationError,
      GarminConnectConnectionError,
    ...
    try:
      api = Garmin(email.strip() or None, (password or "").strip() or None)
...
def _normalize_activity(a: dict[str, Any]) -> dict[str, Any]:
    ...
    type_key = str((at.get("typeKey") or a.get("activityTypeDTO", {}).get("typeKey") or "")).lower()
    name = a.get("activityName") or a.get("activityTitle") or "Aktivität"
    date_key = a.get("startTimeLocal") or a.get("startTimeGMT") or a.get("startTimeInSeconds") or ""
    distance = float(a.get("distance") or 0.0)
    duration = float(a.get("duration") or a.get("movingDuration") or 0.0)
    avg_hr = a.get("averageHR") or a.get("averageHeartRate")
    max_hr = a.get("maxHR") or a.get("maxHeartRate")
    return {
      "id": str(a.get("activityId") or a.get("activityUUID") or ""),
      "name": name,
      "type_key": type_key,
      "start_local": str(date_key),
      "distance_km": round(distance / 1000.0, 2) if distance > 0 else 0,
      "duration_min": round(duration / 60.0, 1) if duration > 0 else 0,
...
def get_recent_activities_from_garmin(
    email: str,
    password: str,
    garmin_tokens: str | None = None,
    limit: int = 6,
    ...
    api = _garmin_login(email, password, garmin_tokens)
    try:
      acts = api.get_activities(0, max(1, min(int(limit), 20)))
    except Exception as exc:
      raise RuntimeError(f"Aktivitäten konnten nicht geladen werden: {exc}") from exc
    out = [_normalize_activity(a) for a in (acts or []) if isinstance(a, dict)]
